...
- `src/renderer/App.vue`: 文档类型识别、Markdown/HTML 预览切换、Text/JSON 编辑器模式、JSON 格式化/单行化、标签页、最近文件、保存和快捷键。
- `src/renderer/components/MarkdownMonacoEditor.vue`: Monaco 编辑器语言模式，由当前文档类型决定。
- `src/renderer/lib/document.ts`: 支持扩展名、文档类型、可预览类型判断、预览渲染、HTML 预览清理、JSON 格式化。
...
Markdown 继续使用 `markdown-it` 渲染并支持 Mermaid、标题目录、图片资源等能力。HTML 使用 iframe 预览，主进程启动只监听 `127.0.0.1` 的 static server：`/preview/:id/index.html` 返回当前编辑器内容，同一路径下的 CSS、JS、图片等资源按 HTML 文件所在目录解析。Text 和 JSON 始终使用编辑器模式，不显示预览面板，`Cmd/Ctrl+P` 不会为这两类文档打开 Preview。
...
sequenceDiagram
  ...
  alt HTML
    ...
    App->>App: 强制 editorVisible=true，禁用预览面板
  end
```
...
3. Text 文件只显示源码编辑器，不显示预览面板。
4. JSON 文件只显示源码编辑器，不显示预览面板；工具栏会出现格式化 JSON 和 JSON 转单行按钮。
